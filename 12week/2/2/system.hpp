...
public:
	...
	explicit System(sf::Vector2f min_point, sf::Vector2f max_point,
		std::vector < particle_t > & particles, std::size_t n_lines, std::size_t n_columns) noexcept :
			m_min_point(min_point), m_max_point(max_point), 
			m_particles(particles)
	{
		initialize(n_lines, n_columns);
	}
...
private:
	...
	void initialize(std::size_t n_lines, std::size_t n_columns);
